...
export function extensionFromPath(pathname) {
...
export function extensionFromUrlParts(pathname, search, hash) {
...
export function isDownloadExtension(ext) {
    return EXECUTABLE_EXTENSIONS.has(ext) || ARCHIVE_EXTENSIONS.has(ext);
}
export function isImagePreviewUrl(parsed) {
...
export function hostsAreRelated(a, b) {
...
export function isShareGoogleVariant(hostname) {
...
export function isPortalInternalHost(hostname, pageHost) {
...
export function hasUrgencyLanguage(text) {
    const lower = text.trim().toLowerCase();
    if (!lower)
        return false;
    return URGENCY_SUBJECT_KEYWORDS.some((kw) => lower.includes(kw));
}
export function hasBrandImpersonationLanguage(text) {
    const lower = text.trim().toLowerCase();
    if (!lower)
        return false;
...
export function checkHrefTextMismatch(label, linkHost, pageHost) {
...
export function checkDownloadTrapHost(hostname) {
    const host = normalizeHostname(hostname);
...
export function extractUrlsFromOnclick(onclick) {
    const urls = [];
    const patterns = [
...
export function finalizeRiskScore(signals) {
    if (signals.length === 0)
        return { dangerous: false, reason: "", confidence: 0 };
...
export function analyzeNavigationTarget(url, pageHost, baseUrl, context) {
...
export function analyzeImageLink(url, imgSrc, pageHost, baseUrl, extra) {
